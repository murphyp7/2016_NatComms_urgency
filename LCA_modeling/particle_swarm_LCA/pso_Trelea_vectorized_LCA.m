...
function [OUT,varargout]=pso_Trelea_vectorized_LCA(functname,D,varargin)
...
for i=1:me  % start epoch loop (iterations)
   ...
   % this section does the plots during iterations
    if (i==1) | (rem(i,5) == 0 )
        fprintf(message,i,gbestval);  %%%% CHANGED SO THAT COMMAND LINE IS UPDATED FOR EVERY FIFTH ITERATION -- PM 29/08/13
    end
    if plotflg==1      
    ...
    chkdyn = 0; %%%%%%%%%%% SET TO ZERO BECUASE THIS SEEMS TO BE IMPAIRING DDM OPTIMISATION...PM 30-08-13
    rstflg = 0; % for dynamic environment checking
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         %%% PM ADDITION SPECIFICALLY FOR LCA FITTING %%%
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         % Constraining recurrent excitation to always be less than leak (otherwise system will never stabilize during pre-stimulus period)
%         for s = 1:size(pos,1)
%             if pos(s,2)>=pos(s,1)
%                 pos(s,2) = pos(s,1)-0.01; % excitation = leak-0.01
%             end
%         end
